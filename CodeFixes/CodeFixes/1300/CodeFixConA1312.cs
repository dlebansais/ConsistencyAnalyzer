namespace ConsistencyAnalyzer
{
    ...
    /// </summary>
    public class CodeFixConA1312 : CodeFix
    {
        /// <summary>
        /// Creates an instance of the <see cref="CodeFix"/> class.
        /// </summary>
        /// <param name="rule">The associated rule.</param>
        public CodeFixConA1312(AnalyzerRule rule)
            : base(rule)
        {
        }

        private async Task<Document> AsyncHandler(Document document, ParameterSyntax syntaxNode, CancellationToken cancellationToken, string newValueText)
        {
            TraceLevel TraceLevel = TraceLevel.Info;
            Analyzer.Trace("CodeFixConA1312", TraceLevel);
            ...
            ParameterSyntax NewNode = syntaxNode.WithIdentifier(SyntaxFactory.Identifier(Leading, newValueText, Trailing));
        ...
        public override CodeAction? CreateDocumentHandler(CodeFixContext context, SyntaxNode root, TextSpan diagnosticSpan)
        {
            ...
            ParameterSyntax Node = Nodes.OfType<ParameterSyntax>().First();
            string ValueText = Node.Identifier.ValueText;
            string NewValueText = NameExplorer.FixName(ValueText, NameCategory.Parameter);

            string CodeFixMessageFormat = new LocalizableResourceString(nameof(CodeFixResources.ConA1312FixTitle), CodeFixResources.ResourceManager, typeof(CodeFixResources)).ToString();
            ...
            var Action = CodeAction.Create(title: FormatedMessage,
                    ...
                    equivalenceKey: nameof(CodeFixResources.ConA1312FixTitle));
