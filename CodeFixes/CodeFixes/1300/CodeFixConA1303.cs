...
/// </summary>
public class CodeFixConA1303 : CodeFix
{
    /// <summary>
    /// Creates an instance of the <see cref="CodeFix"/> class.
    /// </summary>
    /// <param name="rule">The associated rule.</param>
    public CodeFixConA1303(AnalyzerRule rule)
        : base(rule)
    {
    }

    private async Task<Document> AsyncHandler(Document document, StructDeclarationSyntax syntaxNode, CancellationToken cancellationToken, string newValueText)
    {
        TraceLevel TraceLevel = TraceLevel.Info;
        Analyzer.Trace("CodeFixConA1303", TraceLevel);
    ...
    public override CodeAction? CreateDocumentHandler(CodeFixContext context, SyntaxNode root, TextSpan diagnosticSpan)
    {
        ...
        StructDeclarationSyntax Node = Nodes.OfType<StructDeclarationSyntax>().First();
        string ValueText = Node.Identifier.ValueText;
        string NewValueText = NameExplorer.FixName(ValueText, NameCategory.Struct);

        string CodeFixMessageFormat = new LocalizableResourceString(nameof(CodeFixResources.ConA1303FixTitle), CodeFixResources.ResourceManager, typeof(CodeFixResources)).ToString();
        ...
        var Action = CodeAction.Create(title: FormatedMessage,
                ...
                equivalenceKey: nameof(CodeFixResources.ConA1303FixTitle));
