namespace ConsistencyAnalyzer
{
    ...
    /// </summary>
    public class CodeFixConA1310 : CodeFix
    {
        /// <summary>
        /// Creates an instance of the <see cref="CodeFix"/> class.
        /// </summary>
        /// <param name="rule">The associated rule.</param>
        public CodeFixConA1310(AnalyzerRule rule)
            : base(rule)
        {
        }

        private async Task<Document> AsyncHandler(Document document, PropertyDeclarationSyntax syntaxNode, CancellationToken cancellationToken, string newValueText)
        {
            TraceLevel TraceLevel = TraceLevel.Info;
            Analyzer.Trace("CodeFixConA1310", TraceLevel);
            ...
            PropertyDeclarationSyntax NewNode = syntaxNode.WithIdentifier(SyntaxFactory.Identifier(Leading, newValueText, Trailing));
        ...
        public override CodeAction? CreateDocumentHandler(CodeFixContext context, SyntaxNode root, TextSpan diagnosticSpan)
        {
            ...
            PropertyDeclarationSyntax Node = Nodes.OfType<PropertyDeclarationSyntax>().First();
            string ValueText = Node.Identifier.ValueText;
            string NewValueText = NameExplorer.FixName(ValueText, NameCategory.Property);

            string CodeFixMessageFormat = new LocalizableResourceString(nameof(CodeFixResources.ConA1310FixTitle), CodeFixResources.ResourceManager, typeof(CodeFixResources)).ToString();
            ...
            var Action = CodeAction.Create(title: FormatedMessage,
                    ...
                    equivalenceKey: nameof(CodeFixResources.ConA1310FixTitle));
