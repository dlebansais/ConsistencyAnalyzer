namespace ConsistencyAnalyzer
{
    ...
    /// </summary>
    public class CodeFixConA1301 : CodeFix
    {
        /// <summary>
        /// Creates an instance of the <see cref="CodeFix"/> class.
        /// </summary>
        /// <param name="rule">The associated rule.</param>
        public CodeFixConA1301(AnalyzerRule rule)
            : base(rule)
        {
        }

        private async Task<Document> AsyncHandler(Document document, ClassDeclarationSyntax syntaxNode, CancellationToken cancellationToken, string newValueText)
        {
            TraceLevel TraceLevel = TraceLevel.Info;
            Analyzer.Trace("CodeFixConA1301", TraceLevel);
            ...
            ClassDeclarationSyntax NewClassDeclaration = syntaxNode.WithIdentifier(SyntaxFactory.Identifier(Leading, newValueText, Trailing));
        ...
        public override CodeAction? CreateDocumentHandler(CodeFixContext context, SyntaxNode root, TextSpan diagnosticSpan)
        {
            ...
            var Action = CodeAction.Create(title: FormatedMessage,
                    ...
                    equivalenceKey: nameof(CodeFixResources.ConA1301FixTitle));
