namespace ConsistencyAnalyzer
{
    ...
    /// </summary>
    public class CodeFixConA1302 : CodeFix
    {
        /// <summary>
        /// Creates an instance of the <see cref="CodeFix"/> class.
        /// </summary>
        /// <param name="rule">The associated rule.</param>
        public CodeFixConA1302(AnalyzerRule rule)
            : base(rule)
        {
        }

        private async Task<Document> AsyncHandler(Document document, RecordDeclarationSyntax syntaxNode, CancellationToken cancellationToken, string newValueText)
        {
            TraceLevel TraceLevel = TraceLevel.Info;
            Analyzer.Trace("CodeFixConA1302", TraceLevel);
            ...
            RecordDeclarationSyntax NewClassDeclaration = syntaxNode.WithIdentifier(SyntaxFactory.Identifier(Leading, newValueText, Trailing));
        ...
        public override CodeAction? CreateDocumentHandler(CodeFixContext context, SyntaxNode root, TextSpan diagnosticSpan)
        {
            ...
            RecordDeclarationSyntax Node = Nodes.OfType<RecordDeclarationSyntax>().First();
            string ValueText = Node.Identifier.ValueText;
            string NewValueText = NameExplorer.FixName(ValueText, NameCategory.Record);

            string CodeFixMessageFormat = new LocalizableResourceString(nameof(CodeFixResources.ConA1302FixTitle), CodeFixResources.ResourceManager, typeof(CodeFixResources)).ToString();
            ...
            var Action = CodeAction.Create(title: FormatedMessage,
                    ...
                    equivalenceKey: nameof(CodeFixResources.ConA1302FixTitle));
